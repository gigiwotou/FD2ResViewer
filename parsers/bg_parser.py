...
class BgParser(BaseParser):
    ...
    def analysis(self) -> List[Optional[DataBlock]]:
        ...
        while num5 <= num4:
            # 确保不会越界
            if num5 < len(self.main.datablocksBG):
                self.main.datablocksBG[num5] = DataBlock(array[num5], array[num5 + 1] - array[num5])
            num5 += 1
        # 处理最后一个数据块
        if num5 < len(self.main.datablocksBG):
            self.main.datablocksBG[num5] = DataBlock(array[num5], len(self.main.fileDatas) - array[num5])
